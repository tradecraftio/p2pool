...
import p2pool
from p2pool.bitcoin import data as bitcoin_data, script as bitcoin_script, sha256
...
DONATION_SCRIPT = '76a91463753c5b7e79f69644bbbdbf72a4234fb996cff588ac'.decode('hex')
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None, segwit_data=None):
        ...
        if len(blockfinal) == 1:
            ...
            if prevout['txid'] == 0 and prevout['vout'] == 0xffffffff and prevout['amount'] == 0:
                gentx['tx_outs'] = [dict(value=share_data['subsidy'], script=bitcoin_script.create_push_script([cls.gentx_before_refhash + pack.IntType(256).pack(0) + pack.IntType(64).pack(0)]))]
        if segwit_activated:
            gentx['marker'] = 0
            gentx['flag'] = 1
